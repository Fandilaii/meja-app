...
export default function ConfirmationPage() {
  ...
  async function handleShare() {
    if (!reservation || !restaurant) return
    const text = `Saya baru reservasi di ${restaurant.name} via Meja 🪑 ${formatDate(reservation.date)} jam ${reservation.timeSlot}. Book juga: meja-app.vercel.app`
  ...
  return (
    <div className="min-h-screen bg-cream">
      ...
      <div className="px-4 py-5 space-y-4">
        ...
        <button
          onClick={handleShare}
          className="w-full py-3 rounded-[9999px] text-sm font-sans font-medium flex items-center justify-center gap-2 border border-meja-border text-ink hover:bg-sand transition-colors"
        >
          {shared
            ? <><CheckCheck size={15} className="text-forest" /> Tautan Disalin!</>
            : <><Share2 size={15} /> Bagikan Reservasi</>
          }
        </button>
